...
const productos = [
  ...
  },
  {
    ...
    url_imagen: "https://sgfm.elcorteingles.es/SGFM/dctm/MEDIA03/202501/23/00120616500110____1__1200x1200.jpg"
  },
  {
    id: "pascual-semi",
...
const elementos = {
  ...
  abrirGenerador: document.getElementById("open-generator"),
  panelGenerador: document.getElementById("generator-panel"),
  formularioEAN: document.getElementById("ean-form"),
  entradaEAN: document.getElementById("ean-input"),
  contadorEAN: document.getElementById("ean-count"),
  mensajeEAN: document.getElementById("ean-message"),
  productoTemporal: document.getElementById("temporary-product"),
  dialogoEAN: document.getElementById("barcode-dialog"),
  codigoAmpliado: document.getElementById("expanded-barcode"),
  cambiarTema: document.getElementById("theme-toggle"),
...
function iniciarCatalogo() {
  ...
  pintarMarcas();
  enlazarEventos();
  enlazarGeneradorTemporal();
  enlazarAmpliacionEAN();
  enlazarTema();
  renderizarCatalogo();
}
...
function enlazarGeneradorTemporal() {
  const limpiar = () => {
    elementos.productoTemporal.hidden = true;
    elementos.productoTemporal.replaceChildren();
    elementos.entradaEAN.value = "";
    elementos.contadorEAN.textContent = "0/13 dígitos";
    elementos.mensajeEAN.textContent = "";
  };

  elementos.abrirGenerador.addEventListener("click", () => {
    const abrir = elementos.panelGenerador.hidden;
    elementos.panelGenerador.hidden = !abrir;
    elementos.abrirGenerador.setAttribute("aria-expanded", String(abrir));
    if (abrir) elementos.entradaEAN.focus();
    else limpiar();
  });

  elementos.entradaEAN.addEventListener("input", () => {
    elementos.entradaEAN.value = elementos.entradaEAN.value.replace(/\D/g, "").slice(0, 13);
    elementos.contadorEAN.textContent = `${elementos.entradaEAN.value.length}/13 dígitos`;
    elementos.mensajeEAN.textContent = "";
    elementos.productoTemporal.hidden = true;
    elementos.productoTemporal.replaceChildren();
  });

  elementos.formularioEAN.addEventListener("submit", (evento) => {
    evento.preventDefault();
    const ean = elementos.entradaEAN.value.trim();

    if (!/^\d{13}$/.test(ean)) {
      elementos.mensajeEAN.textContent = "Introduce exactamente 13 dígitos.";
      elementos.entradaEAN.focus();
      return;
    }

    if (!esEANValido(ean)) {
      elementos.mensajeEAN.textContent = "El dígito de control no es válido. Revisa el código.";
      return;
    }

    elementos.mensajeEAN.textContent = "Código temporal generado. No se ha guardado.";
    elementos.productoTemporal.innerHTML = `
      <div class="temporary-barcode" data-ean="${ean}" role="button" tabindex="0" aria-label="Ampliar código EAN ${ean}">${generarEAN13SVG(ean)}</div>
      <code>${ean}</code>
      <button type="button" class="close-temporary">Cerrar</button>
    `;
    elementos.productoTemporal.hidden = false;
  });

  elementos.productoTemporal.addEventListener("click", (evento) => {
    if (!evento.target.closest(".close-temporary")) return;
    limpiar();
  });
}

function enlazarAmpliacionEAN() {
  const abrir = (objetivo) => {
    const ean = objetivo?.dataset.ean;
    if (!esEANValido(ean)) return;
    elementos.codigoAmpliado.innerHTML = generarEAN13SVG(ean);
    elementos.dialogoEAN.showModal();
  };

  document.addEventListener("click", (evento) => abrir(evento.target.closest("[data-ean][role='button']")));
  document.addEventListener("keydown", (evento) => {
    if (evento.key !== "Enter" && evento.key !== " ") return;
    const objetivo = evento.target.closest("[data-ean][role='button']");
    if (!objetivo) return;
    evento.preventDefault();
    abrir(objetivo);
  });

  elementos.dialogoEAN.addEventListener("click", (evento) => {
    if (evento.target === elementos.dialogoEAN) elementos.dialogoEAN.close();
  });
}
...
function enlazarTema() {
  elementos.cambiarTema.addEventListener("click", () => {
    ...
    elementos.cambiarTema.setAttribute("aria-label", oscuro ? "Activar modo claro" : "Activar modo oscuro");
  });
}
...
function crearTarjeta(producto) {
  ...
  nodo.querySelector("h2").title = producto.nombre;
  nodo.querySelector("p").title = producto.descripcion;
  pintarImagenProducto(nodo.querySelector(".product-image-box"), producto, categoria);
  const panelCodigo = nodo.querySelector(".barcode-panel");
  panelCodigo.innerHTML = eanValido
  ...
  if (eanValido) {
    panelCodigo.dataset.ean = producto.ean;
    panelCodigo.tabIndex = 0;
    panelCodigo.setAttribute("role", "button");
    panelCodigo.setAttribute("aria-label", `Ampliar código EAN ${producto.ean}`);
  }
...
function pintarImagenProducto(contenedor, producto, categoria) {
  const mostrarPlaceholder = () => {
    ...
    placeholder.append(marca, tipo);
    contenedor.replaceChildren(placeholder);
  };

  if (!producto.url_imagen?.trim()) {
    mostrarPlaceholder();
    return;
  }

  const imagen = document.createElement("img");
  imagen.alt = `${producto.marca} ${producto.nombre}`;
  imagen.loading = "lazy";
  imagen.decoding = "async";
  imagen.addEventListener("error", mostrarPlaceholder, { once: true });
  imagen.src = producto.url_imagen.trim();
